...
describe 'nunjucks', ->
  ...
  before ->
    @nunjucks = accord.load('nunjucks')
    @path = path.join(__dirname, 'fixtures', 'nunjucks')

  it 'should expose name, extensions, output, and compiler', ->
    @nunjucks.extensions.should.be.an.instanceOf(Array)
    @nunjucks.output.should.be.type('string')
    @nunjucks.compiler.should.be.ok
  ...
  it 'should compile a string', (done) ->
    @nunjucks.compile("<p>{{ foo }}</p>\n")
      .catch(should.not.exist)
      .done((res) => should.match_expected(@nunjucks, res({foo: "Test"}), path.join(@path, 'pstring.nunjucks'), done))

  it 'should compile a file', (done) ->
    lpath = path.join(@path, 'precompile.nunjucks')
    @nunjucks.compileFile(lpath)
      .catch(should.not.exist)
      .done((res) => should.match_expected(@nunjucks, res({foo: 'Test'}), lpath, done))

  it 'should client-compile a string', (done) ->
    @nunjucks.compileClient("<h1>{{ foo }}</h1>", {name: 'Test'})
      .catch(should.not.exist)
      .done((res) => should.match_expected(@nunjucks, res, path.join(@path, 'cstring.nunjucks'), done))

  it 'should client-compile a file', (done) ->
    lpath = path.join(@path, 'client.nunjucks')
    @nunjucks.compileFileClient(lpath, {name: 'Noah'})
      .catch(should.not.exist)
      .done((res) => should.match_expected(@nunjucks, res, lpath, done))

  it 'should handle external file requests', (done) ->
    lpath = path.join(@path, 'page.nunjucks')
    @nunjucks.renderFile(lpath)
      .catch(should.not.exist)
      .done((res) => should.match_expected(@nunjucks, res, lpath, done))

  it 'should render with client side helpers', (done) ->
    lpath = path.join(@path, 'complex.nunjucks')
    @nunjucks.compileFileClient(lpath, {name: "Noah"})
      .catch(should.not.exist)
      .done (res) =>
        tpl_string =  "#{@nunjucks.clientHelpers()}; window.aboutTmpl = #{res};"
        tpl = eval.call(global, tpl_string)
        should.match_expected(@nunjucks, tpl, lpath, done)
